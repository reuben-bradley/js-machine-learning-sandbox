...
const Utils = {
    ...
    /**
     * Sort the given output into labels based on the confidence reported.
     * 
     * @param Array responses
     * @param Array labels
     * @return Array
     */
    sortByConfidence: function( responses, labels ) {
        ...
        // Next, ensure there's the same number of items in each
        if ( responses.length !== labels.length ) {
            throw 'Number of responses and number of labels do not match.';
        }
        
        // Now the actual work - first, by combining the responses with their
        //  corresponding labels
        let results = [];
        for ( let i = 0, ln = responses.length; i < ln; i++ ) {
            results.push({
                label: labels[i],
                value: responses[i]
            });
        }
        // Then, sort by largest value first
